...
class GeomObject(BaseModel):
    ...
    bbox: BBOX
    text: Optional[str]
    children: Optional[List["GeomObject"]] = []


GeomObject.update_forward_refs()
...
def stitch_boxes_into_lines(
    boxes, max_x_dist=10, min_y_overlap_ratio=0.8
) -> List[Any]:
    # Copyright (c) OpenMMLab. All rights reserved.
...
def stitch_boxes_into_lines_as_boxes(boxes, min_y_overlap_ratio=0.8):
    # Copyright (c) OpenMMLab. All rights reserved.
    """Like above, but lines are array of boxes"""
    ...
    for i in range(len(x_sorted_boxes)):
        ...
        # Get merged boxes
        for box_group in lines:
            merged_boxes.append([x_sorted_boxes[idx] for idx in box_group])

    # Sort lines based on vertical coordinate
    merged_boxes.sort(
        key=lambda x: min((np.min(token["box"][1::2]) for token in x))
    )

    result = []
    for line in merged_boxes:
        for token in line:
            result.append(token)
    return result


def convert_to_bd(boxes):
    geom_objects = []
    for box in boxes:
        geom_objects.append(
            GeomObject(
...
def convert_to_box(geom_objects):
    boxes = []
        ...
        boxes.append(
            {
                "text": geom_obj.text,
                "box": geom_obj.bbox,
                "children": geom_obj.children
                if hasattr(geom_obj, "children")
                else None,
            }
        )
    return boxes
