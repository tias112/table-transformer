...
def process_bd_results(args):
    (out_dir, results, ann) = (args.out_dir, args.result, args.ann)
    ...
    for result in results.split(','):
        ...
        with open(result, "r") as res_file:
            ...
            results_ordered.extend(data)
    with open(os.path.join(args.out_dir, "coco_results.json"), "w") as f:
        f.write(json.dumps(results_ordered))

def main():
    parser = ArgumentParser(description='COCO Error Analysis Tool')
    parser.add_argument('result',
                        help='result file (json format) path',
                        default='data/nc_ttv/val/coco_detection_output.json')
    parser.add_argument('out_dir', help='dir to save analyze result images')
    parser.add_argument(
        '--ann',
        #default='data/coco/annotations/instances_val2017.json',
        default='data/nc_ttv/val/val.json',
    ...
    process_bd_results(args)
    analyze_results(
        #args.result,
        os.path.join(args.out_dir, "coco_results.json"),
        args.ann,
        args.types,
